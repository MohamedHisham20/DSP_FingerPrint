...
NATIVE_AUDIO_SAMPLING_RATE = 300
number_of_songs = 11

database:List[Dict] = [{} for _ in range(number_of_songs)]
normalized_database: List[Dict] = []
...
def generate_spectrograms(input_folder, get_song_name:bool=False):
    ...
    for directory in directories_names:
        ...
        selected_folder = os.listdir(selected_folder)
    ...
    S_database = librosa.amplitude_to_database(S, ref=np.max)

    if get_song_name:
        spectrograms.append({'song_name':file, 'SG':S_database})
    else: spectrograms.append(S_database)
...
def create_database():    
    songs_spectrograms = generate_spectrograms('Task 5 Data/original data/songs', True)
    vocals_spectrograms = generate_spectrograms('Task 5 Data/original data/vocals')
    music_spectrograms = generate_spectrograms('Task 5 Data/original data/music')
    ...
    for i in range(songs_spectrograms):
        ...
        song_fingerprint = Song_FingerPrint(ssg, vsg, msg, NATIVE_AUDIO_SAMPLING_RATE, song_name)
